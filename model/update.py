...
from model.Attention import SE
...
class UpdateBlock(nn.Module):
    def __init__(self, hidden_dim=128, diff_dim=128):
        super(UpdateBlock, self).__init__()
        self.fusion = SE(features=hidden_dim+diff_dim, M=2, r=2)
        self.gru = SepConvGRU(hidden_dim=hidden_dim, input_dim=hidden_dim+diff_dim+128)  # revise
    ...
    def forward(self, net, inp, mf, hd):
        n, c, h, w = net.size()

        mf = torch.cat([hd, mf], 1) + self.fusion(torch.cat([hd, mf], 1))
